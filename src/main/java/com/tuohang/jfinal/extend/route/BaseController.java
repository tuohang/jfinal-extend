package com.tuohang.jfinal.extend.route;
...
import com.tuohang.jfinal.extend.common.CommonUtil;
...
public class BaseController<M extends Model<M>> extends Controller {
	...
	public void findByPage() {
		...
		String sqlExceptSelect = "from " + getModelClass().getSimpleName()
				+ " where 1 = 1 ";

		if (StrKit.notBlank(getProperty()) && StrKit.notBlank(getKeyword())) {
			sqlExceptSelect += "and " + getProperty() + " like '%"
					+ getKeyword() + "%'";
		}

		sqlExceptSelect += " order by " + getOrderBy() + " " + getOrderType();

		Page<M> pager = getModel(getModelClass()).paginate(
				getParaToInt("pageNumber", 1), getParaToInt("pageSize", 20),
	...
	public boolean isUnique(String tableName, String propertyName, String value) {
		if (StrKit.notBlank(tableName) && StrKit.notBlank(propertyName)
